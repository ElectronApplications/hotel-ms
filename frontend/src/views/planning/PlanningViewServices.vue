...
async function changeServiceDescription(serviceItem: Service, newDescription: string) {
...
async function changeServiceClasses(serviceItem: Service, newClasses: number[]) {
...
async function createService(): Promise<boolean> {
  ...
  const serviceClasses = newServiceClasses.value.map((x) => classes.value[x].id);
  ...
  >
    ...
    <template #classes="item">
      <SelectMultiDynamic
        ...
        :selected="item.data.classes.map((x) => classes.findIndex((y) => y.id == x))"
    ...
    <template #gallery="item">
      ...
      >
        <SelectListDynamic
          ...
          @updateSelection="
            (value) =>
              changeServiceGallery(item.data, value === undefined ? undefined : galleries[value].id)
          "
        />
        <div
    ...
    <template #submit="item">
      <PrimaryButton
        ...
        type="submit"
        :enabled="
          newServiceDescription !== '' && newServicePrice !== '' && newServiceClasses.length !== 0
        "
