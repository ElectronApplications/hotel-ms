...
<template>
  <div v-if="pagination !== undefined"
    ...
    <div class="flex flex-row overflow-hidden rounded-lg bg-surface-light shadow-md dark:bg-surface-dark">
      <template v-for="[index, entry] in pageEntries.entries()" v-bind:key="index">
        <button v-if="entry !== -1" @click="currentPage = entry">
          <div class="px-4 py-2" :class="[
            entry === currentPage
  ...
  <div
    ...
    <form v-if="extraFormRow !== undefined" :id="extraFormRow.formName" @submit.prevent="extraFormRow.formSubmit" />
    ...
    <table class="w-full table-auto bg-slate-100 text-center dark:bg-secondary-dark">
        ...
        <tr>
          <th v-for="column in columns" v-bind:key="column.name" class="py-2">
            <div v-if="column.ordering === true" class="flex w-full flex-row justify-center">
              <button class="flex flex-row justify-center space-x-1" @click="setOrdering(column.name)">
            ...
            <div v-else-if="column.filtering !== undefined" class="flex w-full flex-row justify-center">
              <Popover class="relative">
                ...
                <transition enter-active-class="transition duration-200 ease-out"
                  ...
                  <PopoverPanel class="absolute left-1/2 z-10 mt-1 -translate-x-1/2" v-slot="{ close }">
                    <div
                      ...
                      <button class="text-nowrap py-2" :class="[
                        filtering === undefined ||
                      ...
                      <button v-for="[index, filter] in column.filtering.entries()" v-bind:key="index"
                        class="text-nowrap py-2" :class="[
                          filtering !== undefined &&
      ...
      <tbody class="bg-surface-light dark:bg-surface-dark">
        ...
        <tr v-for="[rowIndex, row] in rows?.entries()" v-bind:key="rowIndex">
          <td v-for="column in columns" v-bind:key="column.name"
            class="border-b border-secondary-light py-4 dark:border-secondary-active-dark">
            <slot :name="column.name" v-bind="{ isFormRow: false, data: row } as TableCardSlot<T>"></slot>
          </td>
        </tr>
      </tbody>
    </table>
