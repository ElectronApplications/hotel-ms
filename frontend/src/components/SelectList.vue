...
export type SelectModel<T extends boolean> = T extends false ? number : number | undefined;
...
<template>
  ...
  >
    ...
    <option v-for="[index, option] in options.entries()" v-bind:key="index" :value="index">
      {{ option }}
    </option>
  </select>
</template>
