...
<template>
  <Popover class="relative">
    ...
    >
      <span
        v-if="model === undefined || model.length === 0"
        class="text-placeholder-light dark:text-placeholder-dark"
        >{{ placeholder }}</span
      >
      <div v-else class="flex flex-row space-x-2">
        <div
          v-for="option in model.slice(0, maxTagsShow)"
          v-bind:key="option"
          class="flex flex-row space-x-2 rounded-full bg-primary-light p-1 dark:bg-primary-dark"
        >
          <span
            class="text-nowrap text-sm text-primary-content-light dark:text-primary-content-dark"
            >{{ options[option] }}</span
          >
          <button @click.stop="switchOption(option)">
            <XMarkIcon
              class="w-[18px] text-primary-content-light dark:text-primary-content-dark"
            />
          </button>
        </div>
